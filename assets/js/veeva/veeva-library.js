...
com.veeva.clm = {
  ...
  getAddresses_Account: function (account, callback) {
    ...
    query =
      'veeva:queryObject(Address_vod__c),fields(ID),where(WHERE Account_vod__c="' +
      account +
      '"),com_veeva_clm_accountAddresses(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_accountAddresses(com.veeva.clm.testResult.common);
  },
  ...
  getAddressFields: function (record, fields, callback) {
    ...
    query =
      "veeva:queryObject(Address_vod__c),fields(" +
      this.joinFieldArray(fields) +
      '),where(WHERE IdNumber="' +
      record +
      '"),com_veeva_clm_addressValues(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_addressValues(com.veeva.clm.testResult.common);
  },
  ...
  getProduct_MySetup: function (type, callback) {
    ...
    query =
      'veeva:queryObject(Product_vod__c),fields(ID),where(WHERE Product_Type_vod__c="' +
      type +
      '"),com_veeva_clm_productMysetup(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_productMysetup(com.veeva.clm.testResult.common);
  },
  ...
  getRecordType_Object: function (object, callback) {
    ...
    query =
      'veeva:queryObject(RecordType),fields(ID),where(WHERE SobjectType="' +
      object +
      '" and IsActive == YES),com_veeva_clm_objectRecordTypes(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_objectRecordTypes(com.veeva.clm.testResult.common);
  },
  ...
  getSurveyQuestions_Survey: function (survey, callback) {
    ...
    query =
      'veeva:queryObject(Survey_Question_vod__c),fields(ID),where(WHERE Survey_vod__c="' +
      survey +
      '"),sort(Order_vod__c,asc),com_veeva_clm_surveyQuestions(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_surveyQuestions(com.veeva.clm.testResult.common);
  },
  ...
  getQuestionResponse_SurveyTarget: function (surveytarget, callback) {
    ...
    query =
      'veeva:queryObject(Question_Response_vod__c),fields(ID),where(WHERE Survey_Target_vod__c="' +
      surveytarget +
      '"),sort(Order_vod__c,asc),com_veeva_clm_targetResponses(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_targetResponses(com.veeva.clm.testResult.common);
  },
  ...
  getSurveyTarget_Account: function (account, survey, callback) {
    ...
    if (survey == null || survey == "") {
      query =
        'veeva:queryObject(Survey_Target_vod__c),fields(ID),where(WHERE Account_vod__c="' +
        account +
        '"),com_veeva_clm_accountSurveyTargets(result)';
    } else {
      query =
        'veeva:queryObject(Survey_Target_vod__c),fields(ID),where(WHERE Account_vod__c="' +
        account +
    ...
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_accountSurveyTargets(com.veeva.clm.testResult.common);
  },
    ...
    window["com_veeva_clm_listPriceTypeId"] = function (result) {
      ...
      if (result.success && result.RecordType && result.RecordType.length > 0) {
        ...
        if (accountOrAccountGroup == null || accountOrAccountGroup == "") {
          query =
            'veeva:queryObject(Pricing_Rule_vod__c),fields(ID,Product_vod__c),where(WHERE RecordTypeId="' +
            listPriceRecordTypeId +
            '" AND Start_Date_vod__c <= "' +
            dateString +
            '" AND End_Date_vod__c >= "' +
            dateString +
            '" AND Product_vod__c IN ' +
            com.veeva.clm.joinStringArrayForIn(ids) +
            "), com_veeva_clm_ordersWithListPrice(result)";
        } else {
          query =
            'veeva:queryObject(Pricing_Rule_vod__c),fields(ID,Product_vod__c),where(WHERE RecordTypeId="' +
            listPriceRecordTypeId +
    ...
    this.getProduct_MySetup("Order", function (result) {
      ...
      if (result.success) {
        ...
        if (orderProducts && orderProducts.length > 0) {
          ...
          recordTypeQuery =
            'veeva:queryObject(RecordType),fields(ID),where(WHERE SobjectType="Pricing_Rule_vod__c" AND Name_vod__c="List_Price_Rule_vod"),com_veeva_clm_listPriceTypeId(result)';
          if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(recordTypeQuery);
          else com_veeva_clm_listPriceTypeId(testResult.listPriceRecordType);
        } else {
          ret.success = true;
          ret.Product_vod__c = [];
          com.veeva.clm.wrapResult("getProduct_OrderActive_Account", callback, ret);
  ...
  getProduct_KitComponents: function (product, callback) {
    ...
    query =
      'veeva:queryObject(Product_vod__c),fields(ID),where(WHERE Product_Type_vod__c="Kit Item" AND Parent_Product_vod__c="' +
      product +
      '"),com_veeva_clm_childKitItems(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_childKitItems(com.veeva.clm.testResult.common);
  },
  ...
  getProductGroup_Product: function (product, callback) {
    ...
    query =
      'veeva:queryObject(Product_Group_vod__c),fields(ID,Product_Catalog_vod__c),where(WHERE Product_vod__c="' +
      product +
      '"),com_veeva_clm_productProductGroups(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_productProductGroups(com.veeva.clm.testResult.common);
  },
  ...
  getLastTenOrders_Account: function (account, callback) {
    ...
    query =
      'veeva:queryObject(Order_vod__c),fields(ID),where(WHERE Account_vod__c="' +
      account +
      '"),sort(Order_Date_vod__c,desc),limit(10),com_veeva_clm_accountLastTenOrders(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_accountLastTenOrders(com.veeva.clm.testResult.common);
  },
  ...
  getOrderLines_Order: function (order, callback) {
    ...
    query =
      'veeva:queryObject(Order_Line_vod__c),fields(ID),where(WHERE Order_vod__c="' +
      order +
      '"),com_veeva_clm_orderLines(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
    else com_veeva_clm_orderLines(com.veeva.clm.testResult.common);
  },
  ...
  getListPrice_Product: function (product, callback) {
    ...
    window["com_veeva_clm_listPriceTypeId_getListPrice_Product"] = function (result) {
      ...
      if (result.success && result.RecordType && result.RecordType.length > 0) {
        ...
        dateString = com.veeva.clm.getCurrentDate();
        query =
          'veeva:queryObject(Pricing_Rule_vod__c),fields(ID),where(WHERE RecordTypeId="' +
          listPriceRecordTypeId +
          '" AND Product_vod__c = "' +
          product +
          '"' +
          ...
          '" AND End_Date_vod__c >= "' +
          dateString +
          '"), com_veeva_clm_productPricingRules(result)';
        if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
        else com_veeva_clm_productPricingRules(com.veeva.clm.testResult.listPrices);
      } else {
        com.veeva.clm.wrapResult("getListPrice_Product", callback, result);
      }
    };
    ...
    recordTypeQuery =
      'veeva:queryObject(RecordType),fields(ID),where(WHERE SobjectType="Pricing_Rule_vod__c" AND Name_vod__c="List_Price_Rule_vod"),com_veeva_clm_listPriceTypeId_getListPrice_Product(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(recordTypeQuery);
    else com_veeva_clm_listPriceTypeId_getListPrice_Product(testResult.listPriceRecordType);
  },
  ...
  getListPrice_Product_Account: function (product, account, callback) {
    ...
    window["com_veeva_clm_get_productDefaultPricingRules"] = function () {
      ...
      groupQuery =
        ...
        '" AND End_Date_vod__c >= "' +
        dateString +
        '"), com_veeva_clm_productDefaultPricingRules(result)';
      if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(groupQuery);
      else {
        // TODO
      ...
      if (result.success) {
        ...
        if (accountGroup != undefined && accountGroup != "") {
          ...
          groupQuery =
            ...
            '" AND End_Date_vod__c >= "' +
            dateString +
            '"), com_veeva_clm_productAccountGroupPricingRules(result)';
          if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(groupQuery);
          else {
            // TODO
    ...
    window["com_veeva_clm_listPriceTypeId_getListPrice_Product_Account"] = function (result) {
      ...
      if (result.success && result.RecordType && result.RecordType.length > 0) {
        ...
        query =
          ...
          '" AND End_Date_vod__c >= "' +
          dateString +
          '"), com_veeva_clm_productAccountPricingRules(result)';

        if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
        else com_veeva_clm_productAccountPricingRules(com.veeva.clm.testResult.listPrices);
      } else {
        com.veeva.clm.wrapResult("getListPrice_Product_Account", callback, result);
      }
    };
    ...
    // 1, fetch list price record type first
    recordTypeQuery =
      'veeva:queryObject(RecordType),fields(ID),where(WHERE SobjectType="Pricing_Rule_vod__c" AND Name_vod__c="List_Price_Rule_vod"),com_veeva_clm_listPriceTypeId_getListPrice_Product_Account(result)';
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(recordTypeQuery);
    else com_veeva_clm_listPriceTypeId_getListPrice_Product_Account(testResult.listPriceRecordType);
  },
    ...
    this.getAppVersion(function (result) {
      ...
      if (result.success) {
        ...
        if (n < 1) {
          ...
          query =
            "veeva:getApprovedDocument(" +
            vault_id +
            "," +
            document_num +
            "),com_veeva_clm_approvedDocument(result)";
          if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
          else com_veeva_clm_appVersion(com.veeva.clm.testResult.common);
        }
      }
      ...
      if (isFeatureMultiProductSupported == false) {
        ...
        window["com_veeva_clm_DocumentTypeId_getDocument"] = function (result) {
          ...
          if (
            result.success &&
            result.Approved_Document_vod__c &&
            result.Approved_Document_vod__c.length == 1
          ) {
            var productsWithDetailGroups;

            //If we have access to detail groups, align products with detail groups
            if (
              detailGroupProducts != undefined &&
              ...
              for (var i = 0; i < detailGroupProducts.length; i++) {
                for (var j = 0; j < productGroups.length; j++) {
                  ...
                  if (
                    detailGroupProducts[i].ID != undefined &&
                    ...
                    result.Approved_Document_vod__c[0].Product_vod__c ==
                      productGroups[j].Product_vod__c
                  ) {
            ...
            if (topicProducts && topicProducts.length > 0) {
              ...
              for (var j = 0; j < topicProducts.length; j++) {
                if (result.Approved_Document_vod__c[0].Product_vod__c === topicProducts[j].ID) {
                  ...
                  if (
                    productsWithDetailGroups != undefined &&
                    productsWithDetailGroups.length > 0
                  ) {
                    for (var i = 0; i < productsWithDetailGroups.length; i++) {
                      if (
                        result.Approved_Document_vod__c[0].Detail_Group_vod__c != undefined &&
                        result.Approved_Document_vod__c[0].Product_vod__c ==
                          productsWithDetailGroups[i].ID &&
                        result.Approved_Document_vod__c[0].Detail_Group_vod__c ==
                          productsWithDetailGroups[i].Detail_Group_vod__c
                      ) {
                        var ret = {};
                        ret.Approved_Document_vod__c = {};
            ...
            if (result.code == 1021) {
              if (result.message.indexOf("Detail_Group_vod__c") >= 0) {
                approvedDocumentQuery =
                  'veeva:queryObject(Approved_Document_vod__c),fields(ID,Product_vod__c),where(WHERE Vault_Instance_ID_vod__c="' +
                  vault_id +
                  '" AND Vault_Document_ID_vod__c="' +
                  document_num +
                  '" AND Status_vod__c="Approved_vod"),com_veeva_clm_DocumentTypeId_getDocument(result)';

                if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(approvedDocumentQuery);
                else com_veeva_clm_DocumentTypeId_getDocument(testResult.approvedDocumentWithId2);
              }
              return;
            }
        ...
        window["com_veeva_clm_getProductGroups"] = function (result) {
          ...
          if (result.success) {
            ...
            approvedDocumentQuery =
              'veeva:queryObject(Approved_Document_vod__c),fields(ID,Product_vod__c,Detail_Group_vod__c),where(WHERE Vault_Instance_ID_vod__c="' +
              vault_id +
              '" AND Vault_Document_ID_vod__c="' +
              document_num +
              '" AND Status_vod__c="Approved_vod"),com_veeva_clm_DocumentTypeId_getDocument(result)';
            ...
            if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(approvedDocumentQuery);
            else com_veeva_clm_DocumentTypeId_getDocument(testResult.approvedDocumentWithId);
          } else {
            if (result.code == 1011) {
              ...
              if (result.success) {
                ...
                com.veeva.clm.getProduct_MySetup("Detail Group", function (result) {
                  if (result.success) {
                    ...
                    query =
                      "veeva:queryObject(Product_Group_vod__c),fields(ID,Product_vod__c,Product_Catalog_vod__c),where(WHERE Product_Catalog_vod__c IN " +
                      groupArray +
                      "),com_veeva_clm_getProductGroups(result)";
                    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(query);
                    else com_veeva_clm_getProductGroups(com.veeva.clm.testResult.productGroups);
                  } else {
  ...
  launchApprovedEmail: function (email_template, email_fragments, callback) {
    ...
    request =
      "veeva:launchApprovedEmail(" +
      email_template +
      "," +
      email_fragments +
      "),callback(com_veeva_clm_launchApprovedEmail)";

    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(request);
    else com_veeva_clm_launchApprovedEmail(com.veeva.clm.testResult.approvedEmailId);
  },
  ...
  getDataForCurrentObject: function (object, field, callback) {
    ...
    request =
      "veeva:getDataForObjectV2(" +
      object +
      "),fieldName(" +
      field +
      "),com_veeva_clm_getCurrentObjectField(result)";
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(request, callback);
    else com_veeva_clm_getCurrentObjectField(com.veeva.clm.testResult.common);
  },
  ...
  getDataForObject: function (object, record, field, callback) {
    ...
    request =
      ...
      "),fieldName(" +
      field +
      "),com_veeva_clm_getObjectField(result)";
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(request, callback);
    else com_veeva_clm_getObjectField(com.veeva.clm.testResult.common);
  },
  ...
  createRecord: function (object, values, callback) {
    ...
    // create record
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(request);
    else com_veeva_clm_createRecord(com.veeva.clm.testResult.common);
  },
  ...
  updateRecord: function (object, record, values, callback) {
    ...
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(request);
    else com_veeva_clm_updateRecord(com.veeva.clm.testResult.common);
  },
  ...
  getUTCdatetime: function (object, record, field, callback) {
    ...
    request =
      "veeva:getDataForObjectV3(" +
      object +
      "),objId(" +
      record +
      "),fieldName(" +
      field +
      "),getUTCdatetime(true),callback(com_veeva_clm_getUTCdatetime)";
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(request, callback);
    else com_veeva_clm_getUTCdatetime(com.veeva.clm.testResult.common);
  },
  ...
  updateCurrentRecord: function (object, values, callback) {
    ...
    request =
      ...
      "),callback(com_veeva_clm_updateCurrentRecord)";
    ...
    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(request);
    else com_veeva_clm_updateCurrentRecord(com.veeva.clm.testResult.common);
  },
  ...
  formatCreateRecords: function (objectArray, valueArray) {
    ...
    for (var ndx = 0; ndx < objectArray.length; ndx++) {
      fullString = fullString.concat(
        com.veeva.clm.generateSaveRecordRequest(objectArray[ndx], valueArray[ndx], "") + ";"
      );
    }
  ...
  formatUpdateRecords: function (objectNameArray, objectIdArray, valueArray) {
    ...
    ret = {};
    if (
      objectNameArray.length != valueArray.length ||
      objectNameArray.length != objectIdArray.length
    ) {
      ret.success = false;
      ret.code = 2003;
      ret.message = "Parameter arrays must be of equal length";
    ...
    for (var ndx = 0; ndx < objectNameArray.length; ndx++) {
      ...
      fullString = fullString.concat(
        com.veeva.clm.generateSaveRecordRequest(objectNameArray[ndx], valueArray[ndx], "") + ";"
      );
    }
  ...
  getFieldLabel: function (object, fields, callback) {
    ...
    request =
      ...
      "), com_veeva_clm_getFieldLabelReturn(result)";
    com.veeva.clm.runAPIRequest(request, callback);
  },
  ...
  getRecordTypeLabels: function (object, callback) {
    ...
    request =
      "veeva:getRecordTypeLabels(" + object + "),com_veeva_clm_getRecordTypeLabelsReturn(result)";
    com.veeva.clm.runAPIRequest(request, callback);
  },
  ...
  getPicklistValueLabels: function (object, field, callback) {
    ...
    request =
      "veeva:getPicklistValueLabels(" +
      object +
      "),field(" +
      field +
      "), com_veeva_clm_getPicklistValueLabelsReturn(result)";
    com.veeva.clm.runAPIRequest(request, callback);
  },
  ...
  getObjectLabels: function (objects, callback) {
    ...
    request =
      "veeva:getObjectLabels(" +
      JSON.stringify(objects) +
      "),com_veeva_clm_getObjectLabelsReturn(result)";
    com.veeva.clm.runAPIRequest(request, callback);
  },
  ...
  request: function (object, callback) {
    ...
    var request =
      "veeva:request(" + JSON.stringify(object) + "),com_veeva_clm_requestReturn(result)";
    com.veeva.clm.runAPIRequest(request, callback);
  },
  ...
  launchSelector: function (presentationSlides, callback) {
    ...
    request =
      "veeva:launchSelector(" +
      JSON.stringify(presentationSlides) +
      "),callback(com_veeva_clm_launchSelector)";

    if (!com.veeva.clm.testMode) com.veeva.clm.runAPIRequest(request);
    else com_veeva_clm_launchSelector(com.veeva.clm.testResult.launchSelectorResult);
  },
  ...
  createMultichannelActivityLine: function (values, callback) {
    ...
    request =
      "veeva:createActivityLine(),value(" +
      JSON.stringify(values) +
      "),com_veeva_clm_createActivityLine(result)";
    com.veeva.clm.runAPIRequest(request, callback);
  },
  ...
  wrapResult: function (apiName, userCallback, result) {
    result = com.veeva.clm.formatResult(result);
    if (result.success) userCallback(result);
    else {
  ...
  runAPIRequest: function (request, callback) {
    if (com.veeva.clm.isEngage()) {
      ...
      window.webkit.messageHandlers.veeva.postMessage({
        message: request,
      });
    } else {
  ...
  isWin8: function () {
    if (navigator.platform.toLowerCase().indexOf("win") >= 0) return true;
    else return false;
  },
  ...
  createRecordsOnExit: () => {
    const actions = JSON.parse(sessionStorage.getItem("store")).state.clickStreamEvents;
    const objectArray = actions.map(() => "Call_Clickstream_vod__c");

    if (actions.length > 0) {
      try {
        const request = com.veeva.clm.formatCreateRecords(objectArray, actions);
        com.veeva.clm.runAPIRequest(request);
      } catch (error) {
        console.error(error);
      }
